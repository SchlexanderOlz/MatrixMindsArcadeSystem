...
void Tetris::mapBlock(const tetris::Block* block, uint x, uint y,  const double x_offset, const double y_offset) const {
  ...
  RectangularArea line(x_pos, y_pos, height, width, Tetris::translateColor(block->getColor()));
  this->engine_->render(std::move(line));
}
...
void Tetris::drawState() const {
  ...
  RectangularArea line_1(x_offset, y_offset, block_height, hor_length, color);
  this->engine_->render(std::move(line_1));

  RectangularArea line_2(x_offset, y_offset, height, block_width, color);
  this->engine_->render(std::move(line_2));

  RectangularArea line_3(ml + offset + 0.1, y_offset, height, block_width, color);
  this->engine_->render(std::move(line_3));

  RectangularArea line_4(x_offset, 0.95 - mt, block_height, hor_length, color);
  this->engine_->render(std::move(line_4));
}
